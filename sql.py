...
client_sql = Groq(api_key=os.environ.get("GROQ_API_KEY"))
...
Make sure whenever you try to search for the brand name or title, the text can be in any case. 
So, make sure to use LIKE '%word%' to find the brand or title in condition. Never use "ILIKE". 
If the user mentions a category like "shoes", use LIKE '%shoes%' in the title field.
...
def generate_sql_query(question, history=[]):
    messages = [
        {
            "role": "system",
            "content": sql_prompt,
...
def data_comprehension(question, context, history=[]):
    messages = [
        {
            "role": "system",
...
def sql_chain(question, history=[]):
    sql_query = generate_sql_query(question, history)
    pattern = "<SQL>(.*?)</SQL>"
    ...
    # Truncate the results to avoid Groq's token limits!
    if not response.empty and len(response) > 10:
        response = response.head(10)

    context = response.to_dict(orient='records')

    answer = data_comprehension(question, context, history)
    return answer
